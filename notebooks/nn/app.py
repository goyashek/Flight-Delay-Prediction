...
# ── PyTorch model classes defined at MODULE level (not inside any function) ──
# This is required so @st.cache_resource can pickle/unpickle them correctly.
...
try:
    ...
    class FlightDelayNet(nn.Module):
        ...
        def forward(self, x):
            return self.head(self.res_block(self.res1(self.stem(x))))

    NN_CLASSES_AVAILABLE = True
except ImportError:
    pass   # torch not installed — NN option simply won't appear


# ── Load XGBoost pipeline (required) ────────────────────────────────────────
@st.cache_resource
def load_artifacts():
...
# ── Load Neural Network (optional, runs ONCE thanks to cache_resource) ───────
@st.cache_resource
def load_nn():
    """
    Loads neural_net.pt exactly once for the lifetime of the server process.
    Returns (nn_model, meta_dict) or (None, None) if files are missing.
    No st.* calls inside — those would cause infinite re-runs.
    """
    if not NN_CLASSES_AVAILABLE:
        return None, None
    if not (os.path.exists('neural_net.pt') and os.path.exists('nn_metadata.json')):
        return None, None

    with open('nn_metadata.json') as f:
        meta = json.load(f)

    nn_model = FlightDelayNet(meta['n_features'])
    ckpt = torch.load('neural_net.pt', map_location='cpu', weights_only=False)
    nn_model.load_state_dict(ckpt['model_state_dict'])
    nn_model.eval()
    return nn_model, meta

nn_model, nn_meta = load_nn()
...
if NN_AVAILABLE:
    model_choice = st.radio(
        "**Prediction model**",
        options=["XGBoost", "Neural Network"],
        horizontal=True,
        help=(
            f"XGBoost: tree-based ensemble (default).  "
            f"Neural Network: Residual MLP, threshold={nn_meta['best_f1_threshold']:.2f}, "
            f"ROC-AUC={nn_meta['roc_auc']:.3f}"
        ),
    )
elif not NN_CLASSES_AVAILABLE:
    model_choice = "XGBoost"
    st.info("💡 Install PyTorch (`pip install torch`) and add `neural_net.pt` to enable the Neural Network option.")
else:
    model_choice = "XGBoost"
...
if predict_btn:
    ...
    if model_choice == "Neural Network":
        ...
        X_transformed = preprocessor.transform(X_input).astype(np.float32)
        t = torch.tensor(X_transformed)
        with torch.no_grad():
            prob = torch.sigmoid(nn_model(t)).item()
        threshold   = nn_meta['best_f1_threshold']
        delayed     = prob >= threshold
        model_label = f"Neural Network (threshold {threshold:.2f})"
    else:
        prob        = model.predict_proba(X_input)[0][1]
        delayed     = prob >= 0.5
        model_label = "XGBoost (threshold 0.50)"
...
st.divider()
st.caption("Models: XGBoost & Neural Network (Residual MLP) trained on US domestic flight data · Target: arrival delay > 15 min")